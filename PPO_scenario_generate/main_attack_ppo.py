...
from PPO import PPO
from env_moving_attack import MassTestingEnv, ownship, navigation_target, obstacle
...
def train(args):
    ...
    ob_line_move=obstacle(id=2, lat=-400, long=2000, sp=6, cog=180,L=200, W=50, Direction=180,risk_range=500)
    ob_line_move2=obstacle(id=3, lat=-200, long=2000, sp=6, cog=-145,L=200, W=50, Direction=-145,risk_range=500)
    ##
    ##attack_
    ob_line_move_front_turn=obstacle(id=2, lat=-400, long=2000, sp=6, cog=180,L=200, W=50, Direction=180,risk_range=500)
    ob_line_move_back_turn=obstacle(id=2, lat=-400, long=0, sp=7, cog=0,L=200, W=50, Direction=180,risk_range=500)
    ob_line_move_side_turn=obstacle(id=2, lat=-1000, long=1500, sp=4, cog=90,L=200, W=50, Direction=90,risk_range=500)
    # if args.obst_id == 'line_static':
    #     ob_list = [ob_line_static]
    if args.obst_id == 'line_move':
        ob_list=[ob_line_move]
    if args.obst_id == 'line_move2':
        ob_list=[ob_line_move2]
    if args.obst_id == 'line_move_front_turn':
        ob_list=[ob_line_move_front_turn]
    if args.obst_id == 'line_move_back_turn':
        ob_list=[ob_line_move_back_turn]
    if args.obst_id == 'line_move_side_turn':
        ob_list=[ob_line_move_side_turn]
        ...
        if i_episode % args.eval_every == 0:
            ...
            for i_eval in tqdm(range(args.num_eval)):
                ...
                if eval_eps_success == 1:
                    success_cnt += 1
                    print(f'Attack target ship at eval env {eval_id} episode {i_eval}!')
                    if success_cnt < 10:
...
def gen_args():
    ...
    parser.add_argument("--obst_id", type=str, default='move')
    parser.add_argument("--dest_id", type=str, default='line')
