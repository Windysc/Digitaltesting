"""
Minimal PPO (actor-critic, clipped surrogate, Monte-Carlo returns) with the
same interface that main_attack_ppo.py / main_ppo.py expect from `PPO`:
...
Additions: `select_action(state, deterministic=True)` for evaluation, GAE-free
but with return normalisation, entropy bonus and gradient clipping.
"""
import numpy as np
import torch
    ...
    def update(self):
        ...
        for _ in range(self.K_epochs):
            ...
            for start in range(0, n, mb):
                ...
                self.optimizer.step()
                stats['policy_loss'] += policy_loss.item(); stats['value_loss'] += value_loss.item()
