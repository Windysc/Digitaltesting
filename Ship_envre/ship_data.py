...
class ShipExperiment:
    ...
    def new_transition(self, s, obs, a, r, other, Rmode, guide):
        ...
        self.guideline[it] = np.vstack([self.guideline[it], guide])
    ...
    def plot_trajectory(self, a, b, iter=0):
        fig = plt.figure()
        """
        ...
        if iter == -1:
            ...
            for i in range(self.iterations + 1):
                ...
                rectan1 = mpatches.Rectangle((self.states[i][len(self.rewardmode[i])-1][0], self.states[i][len(self.rewardmode[i])-1][1]), 4, 8, fill=False, edgecolor='r', linestyle='--')
                ...
                plt.gca().add_artist(rectan1)
                plt.gca().add_artist(circle3)

            plt.legend([line0, line1, line2], ['guideline', 'Vessel', 'Objective Vessel'], loc='upper right')
            ...
            plt.xlabel('X')
            ani = animation(fig, )
            ani.save('trajectory.gif', writer='imagemagick', fps=1)
            plt.show()
